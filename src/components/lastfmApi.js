...
const PLAYLIST_BUFFER_SIZE = 200;
const CACHE_EXPIRATION = 60 * 24; // 24 hours
...
const cache = {
  data: new Map(),
  audio: new Map(),
  set: (key, value, expirationInMinutes = CACHE_EXPIRATION) => {
    const item = {
      value,
      expiry: new Date().getTime() + expirationInMinutes * 60000,
    };
...
const getSpotifyToken = async () => {
  ...
  const data = await response.json();
  spotifyToken = data.access_token;
  cache.set("spotifyToken", spotifyToken, 50);
  return spotifyToken;
};
...
const preloadAudio = (url) => {
  if (cache.audio.has(url)) return cache.audio.get(url);
...
const preloadNextTracks = () => {
  for (let i = currentTrackIndex; i < currentTrackIndex + PRELOAD_COUNT; i++) {
    if (i < playlistBuffer.length) {
      preloadAudio(playlistBuffer[i].audioUrl);
    }
  }
};
    ...
    new Blob(
      [
        ...
        self.onmessage = async function(e) {
          const { type, data } = e.data;
          if (type === 'fetchSpotifyData') {
            const { trackName, artistName, spotifyToken } = data;
            const response = await fetch('${SPOTIFY_BASE_URL}/search?q=track:' + encodeURIComponent(trackName) + ' artist:' + encodeURIComponent(artistName) + '&type=track&limit=5', {
...
worker.onmessage = function (e) {
  ...
  if (type === "spotifyDataFetched") {
    console.log("Spotify data fetched in background:", data);
    // Process and store Spotify data as needed
  }
};

export const fetchPlaylist = async (
  limit = PLAYLIST_BUFFER_SIZE,
  offset = 0,
) => {
  try {
    const jamendoData = await fetchFromJamendo("/tracks/", {
    ...
    const newTracks = jamendoData.results.map((track) => ({
      id: track.id,
      title: track.name,
      artist: track.artist_name,
      albumCover: track.image,
    ...
    playlistBuffer = [...playlistBuffer, ...newTracks];
    preloadNextTracks();

    // Fetch Spotify data in the background
...
export const loadMoreTracks = async (offset) => {
  return fetchPlaylist(PLAYLIST_BUFFER_SIZE, offset);
};

// Additional helper functions (not exported)
const getNextTrack = () => {
  currentTrackIndex++;
  if (currentTrackIndex >= playlistBuffer.length - PRELOAD_COUNT) {
    loadMoreTracks(playlistBuffer.length);
  }
  preloadNextTracks();
  return playlistBuffer[currentTrackIndex];
};
